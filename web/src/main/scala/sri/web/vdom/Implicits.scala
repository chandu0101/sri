...
abstract class LowPri {
  ...
  //  @inline implicit final def _react_fragOption[A <% Frag](xs: Option[A]): Frag = new SeqFrag(xs.toSeq)
...
abstract class Implicits extends LowPri {
  ...
  //
  //  @inline implicit final def _react_attrOptionLike[T[_], A](implicit o: Option[T], t: ReactAttr.ValueType[A]): ReactAttr.ValueType[T[A]] =
  ...
  //  @inline implicit final def _react_styleOptionLike[O, A](implicit t: ReactStyle.ValueType[A]): ReactStyle.ValueType[Option[A]] =
  ...
  @inline implicit def reactNodeInhabitableS(v: String): ReactElement = v.asInstanceOf[ReactElement]
  // Frag
  @inline implicit final def _react_fragReactNode[T <% ReactElement](v: T): Frag = new ReactNodeFrag(v)
  ...
  @inline implicit final def _react_autoRenderN[T <: TopNode](t: ReactTagOf[T]): ReactNode = t.render
  ...
  @inline implicit final def _react_nodeJsArray[A <% TagMod](xs: js.Array[A]): TagMod =
    new SeqNode[A](xs.toSeq)

  @inline implicit final def _react_nodeOptionLike[A](o: Option[A])(implicit f: A => TagMod): TagMod =
    o.fold(EmptyTag)(f)
}
